  ...
  return (
    <div className="max-w-[1000px] mx-auto bg-white p-12 min-h-screen text-[#333] font-sans print:p-4 print:w-full print:max-w-none flex flex-col">
      <div className="flex justify-between items-start mb-10 border-b-2 border-slate-900 pb-8">
         <div className="w-1/2">
            <div className="mb-6">
                {customLogo ? <img src={customLogo} alt="Logo" className="h-20 object-contain" /> : <div className="flex items-center gap-2"><Logo className="h-14 w-auto text-black" /><span className="font-black text-2xl italic tracking-tighter">AMERICAN IRON LLC</span></div>}
            </div>
         ...
         <div className="text-right w-1/3">
             ...
             <div className="text-[11px] text-gray-500 space-y-1">
                 {config.poNumber && <div className="flex justify-end gap-6"><span className="font-black uppercase text-[9px] tracking-widest text-gray-900">P.O. Number</span><span className="font-black text-gray-900">{config.poNumber}</span></div>}
                 ...
                 <div className="flex justify-end gap-6"><span className="font-black uppercase text-[9px] tracking-widest">{config.isInvoice ? 'Due Date' : 'Valid Until'}</span><span className="font-black text-red-600">{config.expirationDate}</span></div>
             </div>
         </div>
      </div>
      ...
      <div className="flex-grow">
        <div className="flex justify-between items-end mb-4">
            ...
            {totalWeight > 0 && <span className="text-[10px] font-bold text-gray-500 bg-gray-50 px-2 py-1">Estimated Ship Wt: {totalWeight.toFixed(1)} {unitLabel}</span>}
        </div>
                        ...
                        <tr key={idx} className="border-b border-gray-100 text-[12px] break-inside-avoid">
                            ...
                            <td className="py-6 pl-4 align-top">
                                <div className="flex items-start gap-4">
                                    <div className="w-16 h-16 flex-shrink-0 bg-white border border-gray-100 flex items-center justify-center overflow-hidden rounded">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <div className="text-[11px] text-gray-600 font-bold uppercase mt-1">{item.desc}</div>
                                        <div className="text-[9px] text-gray-400 mt-2 uppercase font-black tracking-widest">{(item.weight * conversionFactor).toFixed(2)} {unitLabel} / Unit</div>
                                    </div>
                                </div>
                            </td>
                            <td className="py-6 align-top pt-6"><span className="text-[10px] font-black text-slate-600 uppercase bg-slate-100 px-2 py-1 rounded ring-1 ring-slate-200">{item.availability || 'Direct Order'}</span></td>
      ...
      <div className="flex justify-between items-start mt-8 pt-8 border-t-2 border-slate-900 mb-12">
         <div className="w-1/2">
            ...
            <div className="text-[10px] text-gray-500 font-bold uppercase tracking-tight">Ref: AI-ENGINEERING-REF-{config.quoteId.split('-').pop()}</div>
         </div>
         <div className="w-80 space-y-4">
             <div className="flex justify-between items-center text-[11px] font-black text-gray-500 uppercase tracking-widest"><span>Items Subtotal</span><span className="text-gray-900">${subtotalBeforeDiscount.toLocaleString(undefined, {minimumFractionDigits: 2})}</span></div>
             {config.discountPercentage > 0 && <div className="flex justify-between items-center text-[11px] font-black text-slate-500 uppercase tracking-widest"><span className="flex items-center gap-2">Discount Applied ({config.discountPercentage}%)</span><span className="text-red-600">-${discountAmount.toLocaleString(undefined, {minimumFractionDigits: 2})}</span></div>}
             <div className="flex justify-between items-center text-[11px] font-black text-gray-500 uppercase tracking-widest"><span>Logistics & Handling</span><span className="text-gray-900">${logisticsCost.toLocaleString(undefined, {minimumFractionDigits: 2})}</span></div>
             <div className="flex justify-between items-center pt-4 border-t-2 border-slate-900"><span className="text-[14px] font-black text-gray-900 uppercase">{config.isInvoice ? 'Amount Due' : 'Total Estimate'}</span><span className="text-2xl font-black text-gray-900">${total.toLocaleString(undefined, {minimumFractionDigits: 2})}</span></div>
         </div>
      </div>
